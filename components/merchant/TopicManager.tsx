...
export default function TopicManager({
  ...
  function archiveTopic(index: number) {
    ...
    if (topic.id) {
      setArchived((current) => current.some((item) => item.id === topic.id) ? current : [...current, topic]);
    }
    setDirty(true);
    setMessage("");
  }
  ...
  function restoreTopic(topic: TopicDraft) {
    if (!canWrite || active.length >= 8) return;
    if (active.some((item) => item.id === topic.id)) return;
  ...
  if (!locations.length || !selectedLocation) {
    return <div className="merchantCard merchantEmpty">Add a location before configuring review topics.</div>;
  }
    ...
    <div className="merchantTopicLayout">
      <section className="merchantCard merchantTopicControlCard">
        <div className="merchantTopicControlHead">
          <div>
            <span className="merchantEyebrow">LOCATION</span>
            <h2>Choose the customer experience</h2>
            <p>Topics are configured per location. Every QR under that location uses the same active set.</p>
          </div>
          <div className="merchantField merchantTopicLocationSelect">
            <label htmlFor="topic-location">Location</label>
            <select id="topic-location" value={locationId} onChange={(event)=>void selectLocation(event.target.value)} disabled={loading || saving}>
              {locations.map((location) => <option key={location.id} value={location.id}>{location.name}{location.isActive ? "" : " · Paused"}</option>)}
            </select>
          </div>
        </div>
        <div className="merchantTopicRules">
          <span><strong>{active.length}</strong> active topics</span>
          <span>Customers can select up to <strong>3</strong></span>
          <span>Keep labels neutral and factual</span>
        </div>
      </section>
      ...
      <section className="merchantCard merchantTopicEditorCard">
        ...
        {loading ? <div className="merchantTopicLoading">Loading topics…</div> : (
          <div className="merchantTopicRows">
        ...
        {canWrite ? (
          <div className="merchantTopicFooter">
            <button type="button" className="merchantTopicAdd" onClick={addTopic} disabled={active.length >= 8 || loading || saving}>+ Add topic</button>
            <div className="merchantTopicSaveState">
              {dirty ? <span>Unsaved changes</span> : message ? <span className="saved">{message}</span> : <span>Up to 8 active topics</span>}
              <button type="button" className="merchantBtn" onClick={()=>void saveTopics()} disabled={!dirty || saving || loading || active.length < 3 || active.length > 8}>{saving ? "Saving…" : "Save topics"}</button>
            </div>
          </div>
        ) : <div className="merchantTopicReadonly">Viewer access is read only.</div>}

        {error ? <div className="merchantError" role="alert">{error}</div> : null}
      </section>

      {archived.length ? (
        <section className="merchantCard merchantTopicArchiveCard">
          <div className="merchantSectionHead"><div><span className="merchantEyebrow">ARCHIVED</span><h2>Previously used topics</h2><p>Archived topics stay linked to historical review data and can be restored later.</p></div></div>
          <div className="merchantTopicArchiveList">
            {archived.map((topic) => (
              <div key={topic.id ?? topic.label}><span>{topic.icon || "•"}</span><strong>{topic.label}</strong>{canWrite ? <button type="button" onClick={()=>restoreTopic(topic)} disabled={active.length >= 8 || saving}>Restore</button> : null}</div>
            ))}
          </div>
        </section>
      ) : null}
